  ...
  void visitTryApplyInst(TryApplyInst *tai) {
    ...
    if (tai->getSubstitutionMap().empty()) {
      ...
      auto vjpPartialApply = builder.createPartialApply(
        tai->getLoc(), origCallee, substMap, {},
        ParameterConvention::Direct_Guaranteed);
      origCallee = vjpPartialApply;
      originalFnTy = origCallee->getType().castTo<SILFunctionType>();
    ...
    builder.emitScopedBorrowOperation(
      loc, diffFuncInst, [&](SILValue borrowedADFunc) {
                           auto extractedVJP =
                             builder.createDifferentiableFunctionExtract(
                               loc, NormalDifferentiableFunctionTypeComponent::VJP,
                               borrowedADFunc);
                           vjpValue = builder.emitCopyValueOperation(loc, extractedVJP);
                         });
    ...
    for (auto *arg : getOpBasicBlock(tai->getErrorBB())->getArguments().drop_back())
    ...
    // The VJP should be specialized, so no substitution map is necessary.
    auto args = getOpValueArray<8>(tai->getArguments());
    auto *vjpCall = builder.createTryApply(loc, vjpValue, SubstitutionMap(),
                                           vjpArgs,
                                           normalBB, errorBB,
                                           tai->getApplyOptions());
    ...
    // There is no pullback here.
    {
      ...
      bbPullbackValues.push_back(
        trampolineBuilder.createEnum(loc, SILValue(), noneEltDecl,
                                     SILType::getOptionalType(loweredPullbackType)));

      auto pbTupleVal = trampolineBuilder.createTuple(loc,
                                                      tupleLoweredTy, bbPullbackValues);

      auto *succEnumVal =
        buildPredecessorEnumValue(trampolineBuilder, origBB, tai->getErrorBB(), pbTupleVal);
    ...
    // Capture the pullback on normal path and forward result
    {
      ...
      // Get the VJP results (original results and pullback).
      SmallVector<SILValue, 8> vjpDirectResults;
      extractAllElements(normalBB->getArgument(0), trampolineBuilder, vjpDirectResults);
      ArrayRef<SILValue> originalDirectResults =
        ArrayRef<SILValue>(vjpDirectResults).drop_back(1);
      SILValue originalDirectResult =
        joinElements(originalDirectResults, trampolineBuilder, vjpCall->getLoc());
      SILValue pullback = vjpDirectResults.back();
      {
        auto pullbackFnType = pullback->getType().castTo<SILFunctionType>();
        auto pullbackUnsubstFnType =
      ...
      auto tupleLoweredTy = remapType(pullbackInfo.getLinearMapTupleLoweredType(origBB));
      auto pbTupleVal = trampolineBuilder.createTuple(loc,
                                                      tupleLoweredTy, getPullbackValues(origBB));
      
      auto *succEnumVal =
        buildPredecessorEnumValue(trampolineBuilder, tai->getParent(), tai->getNormalBB(),
                                  pbTupleVal);
